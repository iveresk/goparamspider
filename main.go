...
func main() {
	var (
		m               LogMessage
		payload         Payloads
		headers, bodies map[string]string
		err             error
	)
	...
	body := flag.String("B", "", "Set of key=value pairs, to set up body data for a request")
	...
	if *body != "" {
		bodies, err = parseHeaders(*body)
		if err != nil {
			m.Message = "Please check your body data parameters line."
			m.MessageType = "helper"
			m.getLogger()
		}
	}
	...
	case "day", "night":
		params := makeAttack(*mode, *url, *jwt, *paramLevel, *delay, *verbose, *ssl, payload, headers, bodies)
